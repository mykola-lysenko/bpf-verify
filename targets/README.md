...
| Path | Purpose |
|------|---------|
| `ORDER` | Build/verification order, one target name per line. A directory not listed here is disabled but kept for future re-enablement (e.g. the `cnum` toolchain-boundary targets). |
...
- `src` — candidate source paths; the first existing one is compiled (kernel
  releases move files, e.g. `lib/crc16.c` → `lib/crc/crc16.c`). A `{shim}/…`
  path selects a shimmed copy of a kernel file (used by the `proof` targets).
- `suite` — `compat`, `coverage`, or `proof` (see the top-level README). When
  absent the driver derives it (`*_prove`/`BPF_PROVE` → proof, other custom
  harness → coverage, default harness → compat).
...
- `pre_include_shared` — optional list of snippet filenames from a `_shared/`
  directory, concatenated before this target's own `pre_include.h` (supported
  by the driver; no shared snippets are currently in use).
