...
""",
    ...
    *(volatile u32 *)&map->max_entries = 2;
    ...
    return (int)choice + acc;""",
    "bpf_struct_ops_lifecycle_prove": """\
...
EXTRA_EARLY_CFLAGS = {
    ...
    "backtrack_insn_prove": [f"-I{SHIM}/backtrack/include"],
...
#pragma clang attribute push(__attribute__((always_inline)), apply_to=function)
""",
    # ringbuf: model the map/VM/poll/dynptr surfaces, but keep the real
    # ring-buffer helper logic from kernel/bpf/ringbuf.c.
    "ringbuf_prove": """\
#include <linux/errno.h>
#define _LINUX_BPF_H 1
#define _LINUX_BTF_H
...
#define BPF_CALL_3(name, t1, a1, t2, a2, t3, a3) \
    static u64 name(t1 a1, t2 a2, t3 a3)
#define BPF_CALL_4(name, t1, a1, t2, a2, t3, a3, t4, a4) \
    static u64 name(t1 a1, t2 a2, t3 a3, t4 a4)
#define BPF_KEEP_SCALAR(x) asm volatile("" : "+r"(x))
#ifndef container_of
...
# bpf_insn_array_prove exercises the same source and shim surface as
# bpf_insn_array; only the verifier proof body differs.
EXTRA_PRE_INCLUDE["bpf_insn_array_prove"] = EXTRA_PRE_INCLUDE["bpf_insn_array"]
EXTRA_PREAMBLE["bpf_insn_array_prove"] = EXTRA_PREAMBLE["bpf_insn_array"]

# map_in_map_prove exercises the same source and shim surface as map_in_map;
# only the verifier proof body differs.
EXTRA_PRE_INCLUDE["map_in_map_prove"] = EXTRA_PRE_INCLUDE["map_in_map"]
EXTRA_PREAMBLE["map_in_map_prove"] = EXTRA_PREAMBLE["map_in_map"]

# reuseport_array_prove exercises the same source and shim surface as
# reuseport_array; only the verifier proof body differs.
EXTRA_PRE_INCLUDE["reuseport_array_prove"] = EXTRA_PRE_INCLUDE["reuseport_array"]
EXTRA_PREAMBLE["reuseport_array_prove"] = EXTRA_PREAMBLE.get(
    "reuseport_array", ""
)

# bpf_cgrp_storage_prove exercises the same source and shim surface as
# bpf_cgrp_storage; only the verifier proof body differs.
EXTRA_PRE_INCLUDE["bpf_cgrp_storage_prove"] = EXTRA_PRE_INCLUDE[
    "bpf_cgrp_storage"
]
EXTRA_PREAMBLE["bpf_cgrp_storage_prove"] = EXTRA_PREAMBLE.get(
    "bpf_cgrp_storage", ""
)
...
# bpf_inode_storage_prove exercises the same source and shim surface as
# bpf_inode_storage; only the verifier proof body differs.
EXTRA_PRE_INCLUDE["bpf_inode_storage_prove"] = EXTRA_PRE_INCLUDE[
    "bpf_inode_storage"
]
EXTRA_PREAMBLE["bpf_inode_storage_prove"] = EXTRA_PREAMBLE.get(
    "bpf_inode_storage", ""
)

# mprog_prove exercises the same source and shim surface as mprog; only the
...
# bpf_lru_list_prove exercises the same shim helper macros as bpf_lru_list.
EXTRA_PREAMBLE["bpf_lru_list_prove"] = EXTRA_PREAMBLE["bpf_lru_list"]

# bpf_iter_core_prove exercises the same source/pre-include surface as
# bpf_iter; only the verifier proof body and post-source constants differ.
EXTRA_PRE_INCLUDE["bpf_iter_core_prove"] = EXTRA_PRE_INCLUDE["bpf_iter"]

# disasm_prove exercises the same shim and preprocessor surface as disasm.
EXTRA_EARLY_CFLAGS["disasm_prove"] = EXTRA_EARLY_CFLAGS["disasm"]
...
def main():
    ...
    candidates = {
        ...
        "backtrack_insn_prove":  KSRC / "kernel/bpf/backtrack.c",
        ...
        "queue_stack_maps_prove": KSRC / "kernel/bpf/queue_stack_maps.c",
        "bpf_insn_array":        KSRC / "kernel/bpf/bpf_insn_array.c",
        "bpf_insn_array_prove":  KSRC / "kernel/bpf/bpf_insn_array.c",
        "map_in_map":            KSRC / "kernel/bpf/map_in_map.c",
        "map_in_map_prove":      KSRC / "kernel/bpf/map_in_map.c",
        "dispatcher":            KSRC / "kernel/bpf/dispatcher.c",
        "reuseport_array":       KSRC / "kernel/bpf/reuseport_array.c",
        "reuseport_array_prove": KSRC / "kernel/bpf/reuseport_array.c",
        ...
        "bpf_iter_core_prove":   KSRC / "kernel/bpf/bpf_iter.c",
        ...
        "lpm_trie_prove":        SHIM / "kernel/bpf/lpm_trie.c",
        "bpf_lru_list":          SHIM / "kernel/bpf/bpf_lru_list.c",
        "bpf_lru_list_prove":    SHIM / "kernel/bpf/bpf_lru_list.c",
        "bloom_filter":          SHIM / "kernel/bpf/bloom_filter.c",
        "bloom_filter_prove":    SHIM / "kernel/bpf/bloom_filter.c",
        ...
        "verifier_prove":        SHIM / "kernel/bpf/verifier.c",
        "helpers_prove":         SHIM / "kernel/bpf/helpers.c",
        "syscall_prove":         SHIM / "kernel/bpf/syscall.c",
        "syscall_attach_link_prove": SHIM / "kernel/bpf/syscall.c",
        "bpf_struct_ops_prove":  SHIM / "kernel/bpf/bpf_struct_ops.c",
        "bpf_struct_ops_lifecycle_prove": SHIM / "kernel/bpf/bpf_struct_ops.c",
